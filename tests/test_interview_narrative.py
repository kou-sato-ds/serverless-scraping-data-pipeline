...
NARRATIVES = [
    ...
    },
    {
        ...
        {
        "id": "quality_beyond_structure",
        "question": "パイプラインのデータ品質をどう担保しますか",
        "answer": (
            "構造の検証と値の検証を分けています。"
            "DLQ が捕まえるのは取得失敗やパース失敗ですが、"
            "フィードが正常に返ってきて中身だけ劣化しているケースは素通りします。"
            "タイトルが空の記事、重複した link、記事数の激減——"
            "いずれもエラーにならず S3 に書き込まれ、クエリは歪んだ結果を返します。"
            "そこで品質レポートを構造化ログへ出し、"
            "処理は止めずに記録する設計にしました。"
            "劣化したフィードはリトライしても同じ結果になるためです。"
        ),
        "adrs": [10],
        "concern": "data_quality",
    },
        {
        "id": "docs_render_correctly",
        "question": "ドキュメントの品質はどう担保しますか",
        "answer": (
            "内容と構造を分けて検証しています。"
            "ADR が README から辿れるかはテストで守っていましたが、"
            "その文字列が見出しとして描画されるかは見ていませんでした。"
            "実際、姉妹プロジェクトで45個の見出しが引用ブロックに沈み、"
            "目次から消えた状態が1ヶ月以上続いています。"
            "エディタ上では違和感がなく、CI も緑のままでした。"
            "書いた本人には正しく見える事故が最も長く放置されます。"
        ),
        "adrs": [11],
        "concern": "documentation",
    },
        {
        "id": "schema_without_enforcement",
        "question": "スキーマ変更をどう管理しますか",
        "answer": (
            "変更を後方互換と破壊的に分類し、バージョン番号と一致するか検証しています。"
            "S3 は BigQuery と違ってスキーマ強制が無いため、"
            "破壊的な書き込みでも何のエラーも出ません。"
            "露見するのは誰かがクエリしたときです。"
            "メジャーが上がる = 破壊的変更を含む、という約束を守らなければ"
            "バージョン番号は飾りになります。"
            "下流は「2.x なら読める」という前提でクエリを書くからです。"
        ),
        "adrs": [12],
        "concern": "schema_evolution",
    },
        {
        "id": "backfill_without_history",
        "question": "過去データを作り直すにはどうしますか",
        "answer": (
            "データソースの制約で意味が変わります。"
            "RSS は今の記事しか返さないため、過去のフィードは取り直せません。"
            "そこでバックフィルを失敗した時間帯の再生として定義しました。"
            "S3 キーが記事内容から決まるので再実行しても重複せず、"
            "イベントIDをスロット時刻から決定的に作るので元の試行と束ねて追跡できます。"
            "冪等性と観測性を先に作っておいたことが、ここで実利になりました。"
        ),
        "adrs": [13],
        "concern": "backfill",
    },
        {
        "id": "idempotent_but_wasteful",
        "question": "冪等なら毎回全件処理してもよいのでは？",
        "answer": (
            "壊れないことと無駄がないことは別です。"
            "冪等キーで何度書いても結果は同じですが、"
            "毎時全件を書き直せばリクエスト課金を払い続けます。"
            "Lambda は状態を持てないのでウォーターマークを S3 に保存し、"
            "lookback で多めに読んで重複は冪等キーで吸収、"
            "アップロード成功後にだけ前進させています。"
        ),
        "adrs": [14],
        "concern": "incremental",
    },
